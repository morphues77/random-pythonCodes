...
connection = pymysql.connect(host='localhost',user='root',password='1234',db='kobis',charset='utf8mb4',cursorclass=pymysql.cursors.DictCursor)
...
class loggin():
    def __init__(self,window):
        self.window=window
        self.window.title("Image 예측과 분석 AI  시스템 - 로그인창")
        self.window.geometry("400x150+800+300")

        self.ID_data = StringVar()
        self.password_data = StringVar()

        self.login_info = Label(window, text="image operation")
        self.login_info.pack()
        self.login_id_label = Label(window, text="image operation").pack()
        self.username_login_entry = Entry(window, textvariable=self.ID_data)
        self.username_login_entry.pack()
        self.login_pass_label = Label(window, text="image operations").pack()
...
class main_gui():
    ...
    def __init__(self, newWindow):
        ...
        self.window.title("Image operations")
        ...
        self.label3 = Label(self.frame3, text='이미지가 없어서 오류')
        self.button1.pack()
        self.label3.pack()
        ...
        self.framef3_1.pack()
        self.labelf3_2 = Label(self.framef3_1, text=' ')
        self.labelf3_2.pack()
    ...
    def image_change(self):
        ...
        if self.active == 0:
            ...
            try:
                ...
                self.label3.config(text='이미지오류 다음 사진을 누르세요')
            self.active = 1
            self.num = self.num + 1
    ...
    def main(self,image_dir):
        labels = [line.rstrip() for line in tf.gfile.GFile("./workspace/fruits_labels.txt")]
        try:
            with tf.gfile.FastGFile("./workspace/fruits_graph.pb", 'rb') as fp:
